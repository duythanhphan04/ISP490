...
public interface UserGroupRepository extends JpaRepository<UserGroup, String> {
    @Query("SELECT ug.user FROM UserGroup ug WHERE ug.group.group_id = :groupId AND ug.status = 'INACTIVE'")
    List<User> findAllUsersByGroupId(@Param("groupId") String groupId);
    @Query("SELECT ug.group FROM UserGroup ug WHERE ug.user.user_id = :userId AND ug.status = 'INACTIVE'")
